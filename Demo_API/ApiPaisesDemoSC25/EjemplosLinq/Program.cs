...
static void Ejemplo2_Calculadora()
{
    ...
    Func<decimal, decimal, decimal> division = (a, b) => b != 0 ? a / b : throw new DivideByZeroException("No se puede dividir por cero.");
...
static void Ejemplo3_Linq()
{
    ...
    procesos.Add(e => {
        e.FechaEstimadaAscenso = DateTime.Now.AddYears(e.AntiguedadAnios > 5 ? 1 : 3);
        return e;
    });
    ...
    procesos.Add(e => {
        if (e.SalarioMensual < 2000 && e.AntiguedadAnios > 5)
        {
            Console.WriteLine($"Alerta: {e.Nombre} tiene riesgo alto de rotación.");
        }
        return e;
    });
    ...
    procesos.Add(e =>
    {
        // Asignar horas trabajadas según cargo
        int horasAnuales = e.Cargo switch
        {
            "Administrativa" => 40 * 52, // 40 horas/semana
            "Técnico" => 40 * 52,
            "Jefa de Proyecto" => 40 * 52,
            _ => 0 // Otros cargos no definidos
        };
        Console.WriteLine($"{e.Nombre} trabaja {horasAnuales} horas al año.");
        return e;
    });

    procesos.Add(e =>
    {
        e.HorasTrabajadasAnuales = 40 * 52; // 40 horas por semana, 52 semanas al año
        return e;
    });
    ...
    // Proceso: Calcular retención IRPF anual
    procesos.Add(e =>
    {
        if (e.SalarioAnual < 30000)
            e.IRPF = e.SalarioAnual * 0.15m;
        else if (e.SalarioAnual <= 50000)
            e.IRPF = e.SalarioAnual * 0.20m;
        else
            e.IRPF = e.SalarioAnual * 0.25m;
        return e;
    });
    ...
    procesos.Add(e =>
    {
        e.FechaReviionContrato = DateTime.Now.AddMonths(e.AntiguedadAnios < 1 ? 6 : 12);
        return e;
    });
    procesos.Add(e =>
    {
        int mesesRevision = e.AntiguedadAnios < 1 ? 6 : 12;
        e.FechaReviionContrato = DateTime.Now.AddMonths(mesesRevision);
        return e;
    });
    ...
    foreach (var empleado in empleados)
    {
        ...
        Console.WriteLine($"Edad estimada: {empleado.EdadEstimada} años");
    }
}
...
static void Ejemplo5() {
    ...
    // Sintaxis de método
    var nivelesMayores6LinqMethod = niveles
        .Where(nivel => nivel.Length > 6)
        .OrderBy(nivel => nivel)
        .Select(nivel => nivel);
}
...
static void Ejemplo6()
{
    ...
    //                where empleado.Ciudad == "Madrid" && empleado.Nombre.Contains("a")
